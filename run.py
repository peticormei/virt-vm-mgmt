...
def set_mac_to(doc, mac):
    if mac != None and type(mac) == str:
        replace_text(doc, "mac", mac, "address")
        return mac
    return ''

def set_new_hostname(doc, hostname):
    if hostname != None and type(hostname) == str:
...
def clone_harddisk(doc, hostname):
    ...
    path = f"/home/peticormei/Documents/VMs/{filename}"
    copy2(folder, path)
...
class VirtForm(FlaskForm):
    ...
    unity = SelectField("Unity:",choices=[('KiB', 'KiB'),('MiB', 'MiB'),('GiB', 'GiB')], validators=[InputRequired()])
